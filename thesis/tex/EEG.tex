...
In both cases, the \gls{source} is more positively charged than the \gls{sink}. Since the \gls{source} and the \gls{sink} have different electric potentials, they form a \gls{current dipole}. See figure~\ref{fig:dipole} for illustration.
...
\gls{EEG} measures the potential fields as a function of voltage versus time using electrodes placed on scalp~\cite{field_count}. Since voltage is the electric potential difference between two points, one or more reference electrodes are commonly used. Then a voltmeter can be used to measure the differences in voltage between two electrodes, one of which is the reference electrode. See figure~\ref{fig:voltmeter} for a simplified scheme.
...
To make the detection of \glspl{VEP} easier, \glspl{SSVEP} are used. If the visual stimulus is presented at a constant rate and the rate is so fast that the visual pathway does not have enough time to fully recover between stimulus presentations, then the elicited response becomes continuous and it is called the \gls{SSVEP}~\cite{VEP}. See figure~\ref{fig:ssvep} for an example of \glspl{VEP} and \glspl{SSVEP}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{ssvep.png}
	\caption{VEPs and SSVEPs~\cite[p.~259]{VEP}}
	\label{fig:ssvep}
\end{figure}

\gls{SSVEP} is composed of \glspl{VEP} that are elicited one after another in a certain rate. \gls{SSVEP} is continuous and therefore it is easier to detect it in the \gls{EEG} recording than \glspl{VEP}, which are not continuous. Detecting continuous response is easier and more efficient because it is always present in the recording. Thus usually \glspl{SSVEP} are elicited instead of \glspl{VEP} when the goal is to efficiently communicate with external device through brain responses to visual stimuli. %This thesis also focuses on \gls{SSVEP}-based methods used to extract information from \gls{EEG} recording.
