...
To measure the similarity of two \gls{EEG} recordings, sample covariance can be used. Sample covariance measures how much two data sets change together. Thus for two \gls{EEG} recordings $\bar{x}=(x_1,\dots,x_N)$ and $\bar{y}=(y_1, \dots, y_N)$ the sample covariance is
\begin{equation}
	\label{eq:cov}
...
%\end{equation}
Thus when there is \gls{SSVEP} component that corresponds to \gls{target} frequency or its harmonic, then there is positive correlation between the \gls{SSVEP} component and at least one of the reference signals.
