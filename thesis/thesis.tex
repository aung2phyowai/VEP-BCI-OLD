\documentclass[a4paper,12pt]{report}
\usepackage{myStyle}

% Acronyms

\newacronym{EEG}{EEG}{electroencephalography}
\newacronym{BCI}{BCI}{brain-computer interface}
\newacronym{VEP}{VEP}{visual evoked potential}
\newacronym{FFT}{FFT}{fast Fourier transform}
\newacronym{CCA}{CCA}{canonical correlation analysis}
\newacronym{MEG}{MEG}{magnetoencephalography}
\newacronym{fMRI}{fMRI}{magnetic resonance imaging}
\newacronym{PET}{PET}{positron emission tomography}
\newacronym{fNIRS}{fNIRS}{functional near-infrared spectroscopy}

% Packages for layout testing

%\usepackage{showframe}
%\usepackage{layouts}
%\usepackage{layout}
%\printinunitsof{pt}

\includeonly{./tex/abstract, ./tex/title_page, ./tex/appendices, ./tex/introduction, ./tex/EEG, ./tex/BCI, ./tex/conclusion, ./tex/SSVEP_BCI}
\begin{document}

%\layout
%\pagevalues
%\newpage

\include{./tex/title_page}
\include{./tex/abstract}
\tableofcontents
\addcontentsline{toc}{chapter}{Introduction}
\include{./tex/introduction}
\include{./tex/EEG}
\include{./tex/BCI}
\include{./tex/SSVEP_BCI}
\addcontentsline{toc}{chapter}{Conclusion}
\include{./tex/conclusion}


% References
% styles: https://www.sharelatex.com/learn/Bibtex_bibliography_styles

\bibliographystyle{abbrv}
\addcontentsline{toc}{chapter}{References}
\bibliography{thesis}
Internet URLs were valid on ...

\include{./tex/appendices}

\end{document}